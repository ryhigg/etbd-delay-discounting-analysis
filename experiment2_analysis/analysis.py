...
def main():
    global A  # make A global so it can be used in the discounting models
    ...
    for f in files:
        for rep in range(REPS):
            data = pd.read_excel(
                os.path.join(DATA_DIR, f), sheet_name=f"Repetition {rep+1}"
            )
    ...
    ind_ao_data["PropSS"] = ind_ao_data["B1"] / (
        ind_ao_data["B1"] + ind_ao_data["B2"]
    )  # calculate the proportion of choices for the SS alternative

    # get the mean and standard deviation of the response rate and reinforcer rate for each schedule across all repetitions
    mean_sd_data = (
        ind_ao_data.groupby(["File", "Sched"]).agg(["mean", "std"]).reset_index()
    )

    # estimate the indifference points
    # individual AO indifference points
    ind_indiff_pts = {"Rep": [], "LLDelay": [], "IndiffPt": []}
    ...
    for rep in ind_ao_data["Rep"].unique():
        for delay in ind_ao_data["LLDelay"].unique():
            ind_data = ind_ao_data[
                (ind_ao_data["Rep"] == rep) & (ind_ao_data["LLDelay"] == delay)
            ]
            indiff_pt = estimate_indifference_point(
                ind_data["SSMag"].values, ind_data["PropSS"].values
            )
    ...
    mean_indiff_pts = {"LLDelay": [], "IndiffPt": []}
    ...
    for ll_delay in mean_sd_data[("LLDelay", "mean")].unique():
        ...
        indiff_pt = estimate_indifference_point(
            mean_data[("SSMag", "mean")].values, mean_data[("PropSS", "mean")].values
        )
    ...
    for rep in ind_indiff_pts["Rep"].unique():
        ...
        ind_fits_dict["Myerson & Green Residuals"].append(
            myerson_green_fit["residuals"]
        )
